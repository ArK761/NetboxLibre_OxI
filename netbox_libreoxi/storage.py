...
def read_current(root: str, device_id: int) -> tuple[str | None, str | None]:
    ...
    stored_hash = digest.read_text(encoding="ascii").strip() if digest.exists() else sha256(content)
...
def store_if_changed(root: str, device_id: int, content: str) -> dict:
    ...
    old_content, old_hash = read_current(root, device_id)
    ...
    (directory / "current.cfg").write_text(content, encoding="utf-8")
    (directory / "current.sha256").write_text(new_hash + "\n", encoding="ascii")

    return {"changed": True, "hash": new_hash, "old_hash": old_hash, "history": str(history)}
...
def cleanup(root: str, device_id: int, retention_days: int, retention_revisions: int) -> None:
    ...
    current = device_dir(root, device_id) / "current.cfg"
    cutoff = datetime.now(timezone.utc).timestamp() - (retention_days * 86400)
    kept = []
    for path in history:
        if path == current:
            continue
        if path.stat().st_mtime >= cutoff and len(kept) < retention_revisions:
            kept.append(path)
        else:
