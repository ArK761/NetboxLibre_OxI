...
def build_self_pdf(settings, report: dict, subject: str, password: str | None = None) -> bytes:
    """Render the NetBox Self audit as PDF (same look as the configuration audit)."""
    from reportlab.lib import colors
    from reportlab.lib.enums import TA_CENTER
    from reportlab.lib.pagesizes import A4, landscape
    from reportlab.lib.styles import ParagraphStyle
    ...
    document = SimpleDocTemplate(
        ...
        title=subject, author="NetBox LibreOXI", **({"encrypt": _encryption(password)} if password else {}),
    )
    document.build(story, onFirstPage=footer, onLaterPages=footer)
    return buffer.getvalue()
